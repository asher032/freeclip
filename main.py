...
You are a senior short-form video editor. Read the ENTIRE transcript and word-level timestamps to choose the 3–15 best moments for TikTok/IG Reels/YouTube Shorts — the moments people actually watch through. Each clip must be between 15 and 60 seconds long.
...
WHAT MAKES A CLIP PEOPLE ACTUALLY WATCH:
- Opens with a moment that earns the next 3 seconds (curiosity, surprise, or a question that demands an answer)
- Has a clear emotional arc: setup → tension → payoff within the clip
- Stands alone — a viewer with zero context understands it and wants to see what happens next
- Ends on a satisfying conclusion or a moment that makes them want to share it
- Contains real value: a useful insight, a genuine reaction, a story with a twist, or something they haven't seen before

AVOID:
- Generic intros/outros, sponsorship reads, or filler transitions
- Moments that only work if you saw what came before (pronoun-forward openers, answers to earlier questions)
- Clickbait hooks that promise something the clip doesn't deliver
- Clips < 15 s or > 60 s
...
OUTPUT — RETURN ONLY VALID JSON (no markdown, no comments). Order clips by watch-through potential (best to worst). In the descriptions, include a genuine CTA that matches the content:
{{
  "shorts": [
    {{
      "start": <number in seconds, e.g., 12.340>,
      "end": <number in seconds, e.g., 37.900>,
      "video_description_for_tiktok": "<description for TikTok — tease the payoff without spoiling it, 3-5 relevant hashtags>",
      "video_description_for_instagram": "<description for Instagram — tease the payoff without spoiling it, 3-5 relevant hashtags>",
      "video_title_for_youtube_short": "<title for YouTube Short — curiosity-driven, max 100 chars>",
      "viral_hook_text": "<SHORT punchy text overlay (max 10 words) with 1-2 fitting emojis. MUST BE IN THE SAME LANGUAGE AS THE VIDEO TRANSCRIPT. Should tease the value, not exaggerate it. Examples: 'Wait for it... 😳', 'This changes everything 🤯', 'Watch till the end 🎬'>"
    }}
  ]
}}
"""
